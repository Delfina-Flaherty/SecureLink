...
if run_info is not None and not run_info.empty:
    r = run_info.iloc[0]
    st.sidebar.divider()
    st.sidebar.caption("📅 Última corrida del pipeline")
    st.sidebar.write(f"**{r['run_at'].strftime('%Y-%m-%d %H:%M')}**")
    st.sidebar.write(f"Status: `{r['status']}`")
...
if pagina == "Panel General":
    ...
    if metricas is None or metricas.empty:
        st.warning(
            "⚠️ No hay datos aún. Primero hay que correr el pipeline en Airflow "
    ...
    # ── Tabs con análisis profundos ──
    tab_resumen, tab_tend, tab_geo, tab_com = st.tabs(
        ["📊 Resumen", "📈 Tendencias", "🗺️ Geografía", "🏪 Comercios y tarjetas"]
    )

    # ──────────────────────────────────────────────────────────────────────────
    # TAB: RESUMEN — Online vs Swipe, distribución de montos
    # ──────────────────────────────────────────────────────────────────────────
    with tab_resumen:
        st.subheader("💻 Online vs 💳 Presencial (Swipe)")
        st.markdown(
            "Comparación entre transacciones **online** (sin contacto físico con la tarjeta) "
            "y **presenciales** (chip o banda magnética). La pregunta: ¿qué canal es más "
            "vulnerable al fraude? Mirá la diferencia entre los porcentajes — el canal online "
            "es **mucho más riesgoso** porque no hay verificación física de la tarjeta."
        )
        ...
        if online_df is not None and not online_df.empty:
            ...
            with col1:
                ...
                # Extender el eje Y para que la barra alta no quede pegada arriba
        ...
        st.markdown(
            "¿Los fraudes son por montos altos o bajos? El histograma compara la distribución "
            "de montos de transacciones fraudulentas (rojo) vs legítimas (verde), normalizado "
            "a porcentaje (cada barra muestra qué % de cada grupo cae en ese rango). "
            "El rango se acota a $0–$2000 porque por encima son muy pocos casos. "
            "**Si las distribuciones son similares**, el monto no es un buen predictor de fraude. "
            "**Si difieren**, el monto sí ayuda a discriminar."
        )
        ...
        st.divider()
        st.subheader("🎯 Análisis de las reglas de detección")
        st.markdown(
            "**¿Qué tan buenas son las 4 reglas heurísticas del pipeline?** La matriz de "
            "confusión cruza la **predicción** (¿la regla marcó la transacción?) con la "
            "**realidad** (¿era fraude según el label oficial?):\n\n"
            "- ✅ **TP** (arriba-izq): fraude real correctamente detectado\n"
            "- ⚠️ **FP** (arriba-der): legítima marcada por error (falsa alarma)\n"
            "- ❌ **FN** (abajo-izq): fraude que se nos escapó (no detectado)\n"
            "- ✅ **TN** (abajo-der): legítima correctamente ignorada"
        )
        confusion = query("""
            SELECT
        ...
        if confusion is not None and not confusion.empty:
            ...
            with col2:
                ...
                st.metric("Recall (sensibilidad)", f"{recall:.2%}",
                          help="De los fraudes reales, ¿qué % detecta el sistema?")
                st.metric("F1-score", f"{f1:.2%}",
                          help="Media armónica entre precisión y recall")

    # ──────────────────────────────────────────────────────────────────────────
    # TAB: TENDENCIAS — serie temporal anual, mensual, heatmap DOW × hora
    # ──────────────────────────────────────────────────────────────────────────
    with tab_tend:
        st.markdown(
            "El dataset cubre **2010–2019** (~10 años). Esta sección ayuda a entender "
            "**cuándo** ocurre el fraude: en qué años se concentra, en qué meses, "
            "y en qué días/horas de la semana."
        )
        st.subheader("📅 Tendencia anual del fraude")
        st.caption(
            "Las **barras rojas** muestran la cantidad absoluta de fraudes por año. "
            "La **línea azul** muestra la tasa porcentual (% sobre el total del año). "
            "Útil para detectar años atípicos: por ejemplo, 2017 tiene una tasa muy baja "
        ...
        st.caption(
            "Vista de mayor granularidad: cantidad de fraudes mes a mes. Permite identificar "
            "picos estacionales (ej. fin de año, Black Friday) o anomalías puntuales."
        )
        monthly = query("""
        ...
        st.subheader("🕒 Heatmap: día de la semana × hora del día")
        st.markdown(
            "**¿Hay un patrón horario en el fraude?** Cada celda muestra la cantidad de "
            "fraudes detectados para esa combinación día-hora. **Color más oscuro = más fraude.** "
            "Si vemos zonas calientes concentradas (ej. madrugada los fines de semana), "
            "se podría agregar una nueva regla horaria al pipeline."
        )
        heatmap = query("""
    ...
    # ──────────────────────────────────────────────────────────────────────────
    # TAB: GEOGRAFÍA — mapa USA (filtrado) + países
    # ──────────────────────────────────────────────────────────────────────────
    with tab_geo:
        st.info(
            "ℹ️ **Importante**: los datos geográficos representan **dónde se realizó la "
            "transacción** (ubicación del comercio), no el domicilio del titular de la tarjeta. "
            "Las transacciones online no tienen ubicación física y se excluyen del mapa."
        )
        st.subheader("🗺️ Tasa de fraude por estado (EEUU)")
        st.markdown(
            "Mapa coroplético: estados pintados según su **tasa de fraude** "
            "(fraudes / transacciones totales). Zonas más oscuras = mayor proporción de fraude. "
            "Hover sobre cada estado muestra el detalle de transacciones y monto en riesgo."
        )
        state_df = query("""
        ...
        if state_df is not None and not state_df.empty:
            # Filtrar a códigos US de 2 letras válidos (50 estados + DC)
            ...
            if not us_only.empty:
                ...
                st.markdown(
                    "**Dos perspectivas distintas del mismo dato:** "
                    "🥇 *Top 10 por TASA* muestra qué estado tiene la mayor **proporción** "
                    "de fraude (relativo a su volumen). "
                    "💰 *Top 10 por MONTO* muestra dónde se pierde **más dinero** en términos absolutos. "
                    "Un estado puede tener tasa alta pero monto bajo (si es chico), o viceversa."
                )
                col1, col2 = st.columns(2)
                with col1:
            ...
            st.markdown(
                "Los datos incluyen transacciones de tarjetas estadounidenses realizadas en "
                "**comercios fuera de EEUU**. El gráfico muestra los 15 países con más "
                "transacciones; el **color** indica la tasa de fraude en ese país."
            )
            if not intl.empty:
                ...
                if not paises_con_fraude.empty:
                    ...
                    st.info(
                        f"💡 **Insight**: de los {len(intl)} países detectados, solo "
                        f"**{len(paises_con_fraude)}** tienen fraudes etiquetados "
                        f"({nombres}). Italia es el caso más extremo con "
                        f"~45% de las transacciones marcadas como fraude — una tasa "
                        f"~1000x mayor que el promedio global (0.04%)."
                    )
            else:
                st.info("No se detectaron transacciones internacionales.")

    # ──────────────────────────────────────────────────────────────────────────
    # TAB: COMERCIOS Y TARJETAS — MCC, tipos de tarjeta
    # ──────────────────────────────────────────────────────────────────────────
    with tab_com:
        # ── Categorías de comercio (MCC) ──
        st.subheader("🏪 Fraude por categoría de comercio (MCC)")
        st.caption(
            "Dos vistas: arriba = categorías que más **dinero** mueven en fraude; "
            "abajo = categorías con la **tasa** más alta (concentración de riesgo)."
        )
        mcc_df = query("""
        ...
        # ── Tarjetas: marca + tipo + chip + dark web ──
        st.subheader("💳 Análisis por tipo de tarjeta")
        st.markdown(
            "Cruzamos **marca** (Visa, Mastercard, Amex...) con **tipo** "
            "(Credit, Debit, Prepaid). Cada celda del heatmap muestra el % de fraude "
            "para esa combinación. Los **números dentro** de las celdas son la tasa exacta. "
            "Combinaciones más oscuras requieren mayor atención."
        )
        card_df = query("""
        ...
        if card_df is not None and not card_df.empty:
            # Heatmap marca × tipo con tasa de fraude
            ...
            with col2:
                ...
                st.dataframe(
                    by_brand.rename(columns={
                        "card_brand": "Marca", "total": "Txns",
                        "fraud": "Fraudes", "fraud_pct": "% Fraude"
                    }),
                    hide_index=True, use_container_width=True,
                )

            # ── Chip EMV vs Sin chip ──
            st.divider()
            col_chip, col_dw = st.columns(2)

            with col_chip:
                st.subheader("🔐 Impacto del chip EMV")
                st.caption(
                    "El chip EMV reemplaza a la banda magnética con autenticación "
                    "criptográfica. Comparamos la tasa de fraude entre tarjetas con y sin chip."
                )
            ...
            with col_dw:
                ...
                st.caption(
                    "Si el número de tarjeta apareció en filtraciones de la dark web, "
                    "¿se traduce en más fraude? El gráfico compara la tasa de fraude "
                    "entre tarjetas comprometidas y no comprometidas."
                )
                # card_on_dark_web no está en fraud_by_card_type; lo consulto directo
                dw = query("""
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    with tab_perfil:
        ...
        if pd.notna(p["debt_income_ratio"]):
            ...
            col3.metric(
                "Ratio deuda/ingreso", f"{ratio:.2f}",
                delta=f"{'⚠️ Alto' if ratio > 3 else 'OK'}",
                delta_color="inverse" if ratio > 3 else "normal",
                help="Una de las reglas marca como sospechosa toda txn con ratio > 3",
            )
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    if kpis is not None and not kpis.empty:
        ...
        c1.metric("Comercios totales", f"{int(k['total_merchants']):,}",
                  help="Cantidad de merchants distintos detectados en el dataset.")
        ...
        c3.metric("Concentración top 10",
                  f"{(pct_top10 or 0):.1f}%",
    ...
    if top_data is not None and not top_data.empty:
        ...
        top_data["label"] = (
            "Merchant " + top_data["merchant_id"].astype(str)
        ...
        x_label = "Monto perdido en fraude (USD)" if usar_monto else "Cantidad de fraudes"

        if not usar_monto:
            st.warning(
                "ℹ️ El monto exacto perdido no está disponible (requiere reejecutar "
                "`compute_metrics` y `load_to_dwh` en Airflow). Mientras tanto, "
...
# ═══════════════════════════════════════════════════════════════════════════
    ...
    st.title("📂 Análisis de Datasets")
    st.markdown(
        "Esta sección describe **los 5 archivos de entrada** que alimentan el pipeline, "
        "su composición, distribuciones clave y rol en el sistema. Cada dataset aporta "
        "una pieza del rompecabezas: transacciones (los hechos), usuarios y tarjetas "
        "(las dimensiones), labels (la verdad sobre el fraude) y MCC (catálogo de "
        "categorías de comercio)."
    )

    # ── Resumen general de los datasets ──
    st.divider()
    st.header("📋 Resumen de los datasets")
    st.markdown(
        "Los archivos crudos viven en `./data/` y se procesan en el DAG de Airflow. "
        "Cada uno tiene un rol específico en el modelo dimensional del DWH."
    )

    datasets_info = pd.DataFrame([
        {"Archivo": "transactions_data.csv",   "Formato": "CSV",  "Volumen aprox.": "13.3M filas (1.2 GB)",
         "Rol": "Hechos: una fila por transacción", "Clave": "transaction_id"},
        {"Archivo": "users_data.csv",          "Formato": "CSV",  "Volumen aprox.": "2,000 filas",
         "Rol": "Dimensión: titulares de tarjeta", "Clave": "user_id"},
        {"Archivo": "cards_data.csv",          "Formato": "CSV",  "Volumen aprox.": "6,146 filas",
         "Rol": "Dimensión: tarjetas emitidas", "Clave": "card_id"},
        {"Archivo": "train_fraud_labels.json", "Formato": "JSON", "Volumen aprox.": "8.9M registros",
         "Rol": "Ground truth: ¿fue fraude o no?", "Clave": "transaction_id → Yes/No"},
    ...
    # ─────────────────────────────────────────────────────────────────────────
    st.divider()
    st.header("1️⃣ Transactions — la tabla de hechos")
    st.markdown(
    ...
    # ─────────────────────────────────────────────────────────────────────────
    st.divider()
    st.header("2️⃣ Users — perfiles demográficos")
    st.markdown(
        "Cada usuario es un titular de tarjeta. El dataset incluye edad, género, "
        "ingreso anual, deuda total y score crediticio. Estas variables permiten "
        "perfilar al cliente y construir reglas como `debt_income_ratio > 3`."
    )
    ...
    if users_resumen is not None and not users_resumen.empty:
        ...
        c3.metric("Ingreso anual prom.", f"${float(r['ingreso_promedio']):,.0f}")
        c4.metric("Score crediticio prom.", f"{float(r['score_promedio']):.0f}")
        c5.metric("Ratio deuda/ingreso", f"{float(r['dir_promedio']):.2f}")
    ...
    with col2:
        st.subheader("Distribución de score crediticio")
        st.caption(
            "Score crediticio FICO: 300 (peor) – 850 (mejor). "
            "Mayoría de scores entre 600 y 800 indica usuarios financieramente saludables."
        )
        scores = query("""
        ...
        if scores is not None and not scores.empty:
            fig = px.histogram(
                ...
                labels={"credit_score": "Credit Score", "count": "Usuarios"},
            )
            fig.update_layout(height=320, margin=dict(t=20, b=40, l=10, r=10))
            st.plotly_chart(fig, use_container_width=True)
    ...
    # ─────────────────────────────────────────────────────────────────────────
    st.divider()
    st.header("3️⃣ Cards — tarjetas emitidas")
    st.markdown(
        "Cada fila es una tarjeta física emitida. Los usuarios pueden tener varias. "
        "Atributos clave: marca, tipo (Crédito/Débito/Prepago), si tiene chip EMV, "
        "y si apareció en filtraciones de la dark web."
    )
    ...
    if cards_resumen is not None and not cards_resumen.empty:
        ...
        c4.metric("Txns con chip", f"{int(r['con_chip']):,}",
                  help="Cantidad de transacciones realizadas con tarjetas que tienen chip EMV.")
        c5.metric("Txns con tarjeta en dark web", f"{int(r['en_dark_web']):,}",
                  help="Transacciones con tarjetas comprometidas (número filtrado).")
    ...
    # ─────────────────────────────────────────────────────────────────────────
    st.divider()
    st.header("4️⃣ Fraud Labels — la verdad sobre cada transacción")
    st.markdown(
        "El archivo `train_fraud_labels.json` contiene, para cada `transaction_id`, "
        "si esa transacción fue fraude o no según un proceso de etiquetado externo "
        "(banco, equipo de seguridad, investigación post-mortem). "
        "Es el **ground truth** que permite evaluar las reglas y entrenar modelos."
    )
    ...
    if label_resumen is not None and not label_resumen.empty:
        ...
        c4.metric("Ratio desbalance",
                  f"1 : {int(r['legitimas'] / max(r['fraudes'], 1)):,}",
                  help="Por cada fraude, hay esta cantidad de transacciones legítimas. "
                       "Un dataset altamente desbalanceado es típico del fraude — "
                       "esto hace que la métrica accuracy sea engañosa y haya que usar "
                       "precision/recall/F1.")
    ...
    # ─────────────────────────────────────────────────────────────────────────
    st.divider()
    st.header("5️⃣ MCC Codes — catálogo de categorías de comercio")
    st.markdown(
        "**MCC** = Merchant Category Code. Son códigos estandarizados de 4 dígitos "
        "que clasifican a los comercios por rubro (5411 = Grocery Stores, 5812 = Restaurants, "
    ...
    if quality_checks is not None and not quality_checks.empty:
        ...
        with st.expander("🌍 Diversidad geográfica", expanded=True):
            st.markdown(
                f"- **{int(r['estados_distintos'])}** valores distintos en `merchant_state`\n"
                f"- **{int(r['estados_us'])}** códigos de 2 letras (estados USA + DC)\n"
                f"- **{int(r['paises_intl'])}** nombres de países (Mexico, Canada, Italy, etc.)\n"
                f"- **{int(r['sin_estado']):,}** transacciones sin estado (típicamente online)\n\n"
                "El dataset mezcla **estados US** con **países internacionales** en la misma "
                "columna. Por eso el dashboard filtra los códigos US para el mapa choropleth "
        ...
        with st.expander("⚠️ Transacciones con errores", expanded=False):
            st.markdown(
                ...
                "en la columna `errors`\n\n"
                "Esto incluye motivos como `Bad PIN`, `Insufficient Balance`, "
                "`Technical Glitch`, etc. La **Regla 2** del pipeline marca estas "
                "transacciones como sospechosas."
            )

        with st.expander("🧹 Limpieza aplicada por el pipeline", expanded=False):
            st.markdown(
                "Durante `clean_data` se aplicaron las siguientes transformaciones:\n\n"
                "- Conversión de `date` a `transaction_date` (timestamp)\n"
                ...
                "- Normalización de booleanos (`has_chip`, `card_on_dark_web`)\n"
                "- Limpieza de columnas monetarias (`credit_limit`, `yearly_income`, `total_debt`)\n\n"
                f"Resultado final: **{int(r['total_post_clean']):,}** transacciones válidas en el DWH."
            )
